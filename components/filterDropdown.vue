		...
		<view class="nav">
			<block v-for="(item, index) in menu" :key="index">
				...
				>
					...
					<text
						class="iconfont triangle margin-left-xs"
						:style="
							'transform:rotate(' +
								triangleDeg[index] +
								'deg);' +
								(triangleDeg[index] ? 'margin-top:10rpx' : '')
						"
					></text>
				</view>
			</block>
...
export default {
	...
	methods: {
		...
		//展开更多
		showMoreSub(index) {
			this.subData[this.showPage].submenu[this.activeMenuArr[this.showPage][0]].submenu[
				index
			].showAllSub = true;
			this.$forceUpdate();
		},
		...
		selectRadioLabel(page_index, box_index, label_index) {
			...
			if (activeIndex == label_index) {
				...
				if (
					activeIndex != null &&
					activeIndex < this.subData[page_index].submenu[box_index].submenu.length
				) {
